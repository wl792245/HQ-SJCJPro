...
static _Bool nd_busy[2] = {NET_DEVICE_OK, NET_DEVICE_OK};


NET_DEVICE_INFO net_device_info = {NULL, NULL, NULL, NULL, NULL,
									0, 0, 0, 0, 100,
									0, 0, 0, 0, 0, 1, 0};  //100-模组最短数据稳定发送时间间隔发送
...
//M6312通用板   1-vcc    2-gnd  3-TXD  4-RXD 5-nd-pwrk 6-vcc 7-gnd
...
__inline static _Bool NET_DEVCIE_IsBusReady(_Bool type)
{
	...
	_Bool result = NET_DEVICE_Err;
	
	if(nd_busy[type] == NET_DEVICE_OK)
	{
		nd_busy[type] = NET_DEVICE_Err;
		
		result = NET_DEVICE_OK;
	}
...
//			
//			UsartPrintf(USART_DEBUG, "经度:%s,纬度:%s,海拔:%f,速度:%f,卫星数:%d\r\n", lon, lat, *altitude, *speed, *satellite);
//				
//			result = 0;
//		}
//	}
		...
		case M6312:
			switch(net_device_info.init_step)
			{
				...
				case 1:
					...
					if(!NET_DEVICE_SendCmd("AT+CPIN?\r\n", "READY", 400))  //确保SIM卡PIN码解锁，返回READY，表示解锁成功
						net_device_info.init_step++;
				break;
				...
				case 3:
					UsartPrintf(USART_DEBUG, "4. AT+CGREG=2\r\n");
					if(!NET_DEVICE_SendCmd("AT+CGREG=2\r\n", "OK", 400))  //使网络注册状态，位置消息的主动回显有效
						net_device_info.init_step++;
					break;
					
				case 4:
					UsartPrintf(USART_DEBUG, "5. AT+CGREG?\r\n");
					...
					break;
				case 5:
					UsartPrintf(USART_DEBUG, "6. AT+CGACT=1,1\r\n");
					if(!NET_DEVICE_SendCmd("AT+CGACT=1,1\r\n", "OK", 400))  //激活
						net_device_info.init_step++;
					break;
				case 6:
					UsartPrintf(USART_DEBUG, "7. AT+CGATT=1\r\n");
					if(!NET_DEVICE_SendCmd("AT+CGATT=1\r\n", "OK", 400))  //附着GPRS业务
						net_device_info.init_step++;
					break;
				case 7:
					UsartPrintf(USART_DEBUG, "8. AT+CMMUX=0\r\n");
					if(!NET_DEVICE_SendCmd("AT+CMMUX=0\r\n", "OK", 400))  //必须为单连接，不然平台IP都连不上
						net_device_info.init_step++;
					break;
				case 8:
					UsartPrintf(USART_DEBUG, "9. AT+CMHEAD=1\r\n");
					if(!NET_DEVICE_SendCmd("AT+CMHEAD=1\r\n", "OK", 400))  //显示IP头
						net_device_info.init_step++;
					break;
				case 9:
					UsartPrintf(USART_DEBUG, "10. AT+IPSTART\r\n");
					if(!NET_DEVICE_SendCmd("AT+IPSTART=\"TCP\",\"47.104.137.7\",3306\r\n", "CONNECT", 400))  //显示IP头
						net_device_info.init_step++;
					break;
				case 10:
		#if(NET_TIME_EN == 1)
...
//	if(data_ptr_t != NULL)
//	{
//		data_ptr_t = strstr(data_ptr_t, "+MIPURC:");				//搜索“+MIPURC”头
...
//				break;
//			}
//			else if(strstr(net_device_info.cmd_resp, "CLOSE"))					//已关闭
//			{
...
//				break;
//			}
//			else if(strstr(net_device_info.cmd_resp, "INITIAL"))				//已关闭
//			{
